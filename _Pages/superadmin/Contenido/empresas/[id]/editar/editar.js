...
export default function EditarEmpresaPage({ id }) {
  ...
  useEffect(() => {
    Promise.all([obtenerEmpresa(id), obtenerMonedas(), obtenerModulos()])
      .then(([emp, mon, mods]) => {
        if (emp) {
          setForm({
            nombre:       emp.nombre       ?? "",
            ...
            ciudad:       emp.ciudad       ?? "",
            moneda_id:    emp.moneda_id    ?? "",
            estado:       emp.estado       ?? "activa",
          })
          setModulosSeleccionados(emp.empresa_modulos?.map(em => em.modulo_id) ?? [])
        }
        setMonedas(mon)
        setModulos(mods)
        setCargando(false)
  ...
  function toggleModulo(mid, modo) {
  ...
  function toggleTodoModo(modo) {
    ...
    if (todosOn) {
      ...
      setModulosSeleccionados(prev => {
        ...
        return [...base, ...ids.filter(x => !base.includes(x))]
      })
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    router.push(`/superadmin/empresas/${id}/ver`)
  }
  ...
  return (
    <div className={s.page}>
      <div className={s.topRow}>
        <button className={s.btnBack} onClick={() => router.push(`/superadmin/empresas/${id}/ver`)}>
          <ion-icon name="arrow-back-outline" /> Ver empresa
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className={s.formGrid}>
        ...
        <div className={s.formFooter}>
          <button type="button" className={s.btnCancelar} onClick={() => router.push(`/superadmin/empresas/${id}/ver`)}>
            Cancelar
          </button>
