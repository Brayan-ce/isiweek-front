...
async function actualizarCotizacion(id, empresaId, body) {
  try {
    const res = await apiFetch(`/api/pos/cotizaciones/editar/${id}/${empresaId}`, {
      method: "PUT",
...
export default function Editar({ id }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      setDatos(d)
      if (cot && !cot.error) {
        setClienteId(cot.cliente_id ? String(cot.cliente_id) : "")
        setClienteNombre(cot.cliente?.nombre ?? "")
        setNotas(cot.notas ?? "")
        setDescuento(Number(cot.descuento) > 0 ? String(cot.descuento) : "")
        setItems(cot.cotizacion_items.map(i => ({
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.topBar}>
        ...
        <h1 className={s.titulo}>Editar cotizacion #{String(id).padStart(5, "0")}</h1>
      </div>
