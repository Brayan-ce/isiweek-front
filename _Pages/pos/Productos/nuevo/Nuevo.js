...
const FORM_VACIO = {
  ...
  stock: "0", itbis_pct: "18", itbis_habilitado: true, categoria_id: "", marca_id: "", activo: true,
}
...
export default function Nuevo() {
  ...
  async function inicializar() {
    setCargando(true)
    const [datos, header] = await Promise.all([
  ...
  useEffect(() => {
    if (!empresaId) return
    if (codigoManualRef.current || !form.nombre.trim()) {
      if (!form.nombre.trim()) setCodigoEstado(null)
      return
    }
    ...
    codigoTimeoutRef.current = setTimeout(async () => {
      try {
        const params = new URLSearchParams({ nombre: form.nombre })
        const res = await apiFetch(`/api/pos/productos/siguiente-codigo/${empresaId}?${params}`)
        if (!res.ok) { setCodigoEstado(null); return }
        const data = await res.json()
        if (!data.codigo) { setCodigoEstado(null); return }
        setF("codigo", data.codigo)
        setCodigoEstado("ok")
      } catch { setCodigoEstado(null) }
    }, 600)
  ...
  async function handleGuardar() {
    ...
    const r = await apiFetch(`/api/pos/productos/${empresaId}`, {
      method: "POST",
    ...
    if (imgFile) {
      ...
      await apiFetch(`/api/pos/productos/${res.id}/imagen`, { method: "POST", body: fd }).catch(() => {})
      setSubiendoImg(false)
    }

    setGuardando(false)
    router.push(`/pos/productos/${res.id}/ver`)
  }
    ...
    <div className={s.page}>
      ...
      <div className={s.topBar}>
        ...
        <div className={s.topBarTitle}>
          <div className={s.titleIcon}><ion-icon name="cube-outline" /></div>
          <div>
            <div className={s.titulo}>Nuevo producto</div>
            <div className={s.subtitulo}>Completa los datos para registrar el producto</div>
          </div>
        </div>
      </div>
      ...
      <div className={s.contenido}>
        ...
        <div className={s.formWrap}>
            ...
            <div className={s.formGrid}>
              <div className={`${s.field} ${s.spanFull}`}>
                ...
                <input
                  ...
                  value={form.nombre}
                  onChange={e => { codigoManualRef.current = false; setF("nombre", e.target.value) }}
                  autoFocus
                />
              </div>
              <div className={`${s.field} ${s.spanFull}`}>
              ...
              <div className={`${s.field} ${s.spanFull}`}>
                ...
                <div className={s.codigoWrap}>
                  ...
                  />
                  {codigoEstado === "verificando" && <span className={s.codigoBadge}><span className={s.spinnerSmall} /></span>}
                  {codigoEstado === "ok"          && <span className={`${s.codigoBadge} ${s.codigoBadgeOk}`}><ion-icon name="checkmark-circle-outline" /></span>}
                </div>
                {!codigoManualRef.current && form.nombre.trim() && (
                  <span className={s.codigoHint}>Se generará automáticamente desde el nombre</span>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Estado — fix: ambos botones muestran su color activo */}
          <div className={s.card}>
          ...
          <div className={s.acciones}>
            <button className={s.btnCancelar} onClick={() => router.back()}>Cancelar</button>
            <button className={s.btnGuardar} onClick={handleGuardar} disabled={guardando || subiendoImg}>
              {guardando || subiendoImg
                ...
                : <><ion-icon name="add-circle-outline" />Crear producto</>
              }
            </button>
          </div>
